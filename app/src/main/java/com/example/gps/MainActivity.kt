            ...
            REQUEST_CODE->{
                if (grantResults.size>0){
                    if (grantResults[0]==PackageManager.PERMISSION_GRANTED)
                        Toast.makeText(this@MainActivity,"Permision Granted", Toast.LENGTH_SHORT).show()
                    else Toast.makeText(this@MainActivity,"Permision denied", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
